...
class Ion_auth_model extends CI_Model
{
     ...
     * @return bool
     */
    public function change_username($identity, string $username): bool
    {
        ...
        // check result for current username by identity
        if ($query->num_rows() !== 1) {
            return false;
        }
        // check current username vs new username
        if ($query->row() === $username) {
            return false;
        }

        // check if new username is free to use
        if ($this->username_check($username)) {
            return false;
        }
        // update username
        return $this->db->update($this->tables['users'], ['username' => $username], [$this->identity_column => $identity]);
	}
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description,'.$this->tables['groups'].'.default_location as location')
